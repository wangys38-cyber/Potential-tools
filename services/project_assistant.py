...
def build_system_prompt(snap):
    ...
    lines.append(
        ...
        '3. 用户要邮件 / 周报 / 放行报告 / 牛马笔记时，直接输出可一键复制的 Markdown，严格控制总字数在500字以内，绝对不能被截断，格式如下：\n'
        '   第一行：【趋势结论】一句话总结近14天趋势（新增/解决/风险变化）。\n'
        '   第二行：【总体状态】CR总数/未解决/未解决BC/FAIL模块数，一句话。\n'
        '   【新增 BC（不含 iOS APP）】Markdown 表格，只列状态为 New 且模块不包含 IOS_APP/iOS 的 Blocker/Critical，最多8条；'
        '列：严重度|CR单号|标题（不超过30字）|经办人；Blocker在前。\n'
        '   【iOS APP 新增 BC】Markdown 表格，只列状态为 New 且模块包含 IOS_APP/iOS 的 Blocker/Critical，最多8条；列同上。\n'
        '   - 不要额外解释，不要空泛套话，标题超长用省略号截断，确保输出完整不被截断。\n'
