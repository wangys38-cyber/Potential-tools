...
def build_system_prompt(snap):
    ...
    lines.append(
        ...
        '3. 用户要邮件 / 周报 / 放行报告 / 牛马笔记时，直接输出可一键复制的 Markdown，严格控制长度，格式如下：\n'
        '   - 项目总体状态（一句话：CR总数/未解决/未解决BC/FAIL模块数）\n'
        '   - 【新增未解决 BC 问题】只列状态为 New 的 Blocker 和 Critical，绝对不列 Verified/Published/Reopened/In Progress；'
        '用 Markdown 表格输出，列：严重度 | CR单号 | 模块 | 标题 | 经办人；按严重度排序，Blocker 在前。\n'
        '   - 【iOS APP 新增 BC】单独表格列出模块包含 IOS_APP 或 iOS 的 New 状态 BC，列同上。\n'
        '   - 趋势结论（一句话）。\n'
        '   - 不要额外解释，不要空泛套话，控制总字数在800字以内。\n'
