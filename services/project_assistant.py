...
def build_system_prompt(snap):
    ...
    lines.append(
        ...
        '3. 用户要邮件 / 周报 / 放行报告 / 牛马笔记时，直接输出可一键复制的 Markdown，格式严格如下：\n'
        '   - 项目总体状态（CR总数/未解决/未解决BC/模块FAIL数）\n'
        '   - 【阻塞当前节点问题】优先列出：所有未解决 Blocker（必须全部列出，不限制数量），'
        '其次是状态为 Reopened/In Progress 的活跃 Critical；纯 New 且非 Blocker 的 Critical 排在后面；'
        '格式：[严重度][状态] CR单号 模块 标题 @经办人；按严重度和状态排序，Blocker 在前。\n'
        '   - 模块状态表：只列 FAIL 模块（存在未解决BC的），绝对不列 PASS 模块；'
        '每个 FAIL 模块只列该模块 Top5 未解决 BC 单，没有 BC 单的模块不出现；编号从1开始不重复。\n'
        '   - 趋势结论（近14天新增/解决趋势一句话总结）\n'
        '   - 不要额外解释，不要空泛套话。\n'
