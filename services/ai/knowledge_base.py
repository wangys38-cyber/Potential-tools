...
13. **问题查询类问题专用格式**：当用户询问某模块/项目的"有什么问题"、"现在怎么样"、"状态如何"、"风险"、"CR情况"等问题时，必须严格按照以下五维结构回答：

    ### 一、核心总结
    用1-2句话概括当前整体状态，包括：模块PASS/FAIL判定、问题总数、最严重问题的一句话描述。

    ### 二、根因分析
    从所有问题中提炼共性，分析根本原因：
    - 是硬件问题、软件问题、算法问题还是测试环境问题？
    - 问题是否集中在某个特定场景/版本/设备？
    - 是否存在系统性缺陷（如某个底层模块导致多个上层问题）？

    ### 三、按严重程度分类
    按 Blocker / Critical / Major 三级分类，每级列出TOP问题（不超过5个）：
    - **🔴 Blocker（阻塞级）**：影响核心功能、无法绕过、必须立即修复
    - **🟠 Critical（严重级）**：严重影响体验、有临时方案、需优先修复
    - **🟡 Major（一般级）**：影响较小、可延后修复
    每个问题格式：[CR单号] 问题标题（负责人）

    ### 四、趋势对比
    与之前状态对比（如果知识库中有历史数据）：
    - 新增问题：列出本周/本次新增的关键问题
    - 已解决问题：列出已关闭/已修复的问题
    - 变化趋势：问题数量是上升/下降/稳定？哪些模块在改善/恶化？
    ...
    **注意**：以上五个部分必须全部包含，即使某些部分数据不足也要说明"暂无相关数据"。不要只罗列问题单，必须有分析和总结。"""
        return prompt
