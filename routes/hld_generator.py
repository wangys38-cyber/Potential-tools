...
def generate_lld_detail(feature):
    ...
    lines.append("```mermaid")
    lines.append("graph TD")
    lines.append("    A[" + sanitize_name(feature['name']) + "_init] --> B[hal_init]")
    lines.append("    A --> C[storage_open]")
    lines.append("    A --> D[timer_create]")
    lines.append("    E[" + sanitize_name(feature['name']) + "_main_loop] --> F[wait_event]")
    lines.append("    F --> G[handle_user_input]")
    lines.append("    F --> H[process_hal_data]")
    lines.append("    F --> I[handle_bt_data]")
    lines.append("    F --> J[do_periodic_task]")
    lines.append("    G --> K[ui_update]")
    lines.append("    H --> L[do_business_logic]")
    lines.append("    L --> M[save_to_storage]")
    lines.append("    I --> N[sync_to_companion]")
    lines.append("    J --> M")
    lines.append("    J --> N")
    lines.append("```")
    lines.append("")
